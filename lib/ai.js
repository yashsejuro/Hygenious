...
export async function analyzeHygieneImage(imageBase64, mimeType = 'image/jpeg') {
    ...
    const prompt = `
      ...
      {
        "isAppropriate": "boolean (true if the image is a valid space/hygiene context, false if it contains 18+, explicit, NSFW content, or is completely irrelevant like a selfie/person)",
        "overallScore": "number (0-100)",
        "cleanliness": "number (0-100)",
        "organization": "number (0-100)",
        "safety": "number (0-100)",
        "assessment": "string (A brief summary of your findings, or the reason for rejection if inappropriate)",
        "issues": [
          {
      ...
      0.  **Content Moderation (CRITICAL):** First, check if the image is safe and relevant. If it contains 18+, explicit, NSFW content, or is a selfie/person instead of a facility/space, set "isAppropriate" to false, set all scores to 0, explain the rejection in the "assessment", and provide empty arrays for "issues" and "recommendations". If it is appropriate, set "isAppropriate" to true and proceed with the steps below.
      1.  **Scoring:**
